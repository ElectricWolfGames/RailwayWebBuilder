...
[PageTitle("DieselPowers.html")]
[Navigation(NavigationTypes.Main, 2)]
public class DieselPowerListPage : PageDetails
{
    public DieselPowerListPage()
    {
        WebPage = new WebPage(this);
        DisplayTitle = "Diesel Locomotives by Power Output";
        MenuTitle = "Diesel Power";
    ...
    public override void CreatePage()
    {
        ...
        var byPower = allEntries
            .Where(e => !string.IsNullOrEmpty(e.Power))
            .GroupBy(e => e.Power)
            .OrderBy(g => g.Key)
            .ToList();
        ...
        WebPage.Append("<p class='lead mb-4'>British diesel and electric locomotive classes grouped by power output.</p>");
        WebPage.Append("<div class='row'>");

        foreach (var group in byPower)
        {
  ...
  <div class='card h-100'>
    <div class='card-body'>
      <h6 class='card-title mb-1'><a href='DieselPowers/{slug}.html'>{group.Key}</a>{collBadge}</h6>
      <p class='text-muted small mb-0'>{group.Count()} class(es)</p>
        ...
        foreach (var group in byPower)
        {
            ...
            {
                RefTitle = group.Key,
                CategoryLabel = "Power Output",
                Entries = group.ToList(),
                OutputSubPath = Constants.DieselPowers,
